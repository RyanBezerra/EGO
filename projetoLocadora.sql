...
FROM
    veiculo v
    JOIN contrato c ON v.id = c.idVeiculo
    JOIN cliente cl ON c.codigoCliente = cl.codigo
WHERE
    v.ano BETWEEN 2010 AND 2018
ORDER BY
    cl.uf DESC;
    ...
    SELECT
    cl.uf,
    COUNT(cl.codigo) AS quantidade_clientes
FROM
    cliente cl
WHERE
    cl.uf IN ('BA', 'SE', 'PE', 'PB', 'CE', 'PI')
GROUP BY
    cl.uf
ORDER BY
    cl.uf;
    
    CREATE VIEW vw_veiculos_baratos AS
SELECT
    c.nome AS nome_cidade,
    f.bairro AS bairro_filial,
    v.placa,
    v.modelo,
    v.ano
FROM
    ...
    JOIN cidade c ON f.idCidade = c.codigo
WHERE
    v.valorDiaria < 120.00
    AND v.marca IN ('Hyundai', 'Honda', 'Fiat');
SELECT * FROM vw_veiculos_baratos;
...
FROM
    contrato c
    JOIN cliente cl ON c.codigoCliente = cl.codigo
    JOIN veiculo v ON c.idVeiculo = v.id
    JOIN filial f ON v.codFilial = f.codigo
WHERE
    MONTH(c.dataDevolucao) = 9
    AND YEAR(c.dataDevolucao) = 2021;
